...
| `playwright-cli`, `playwright-best-practices`, `playwright-generate-test`, `e2e-testing-patterns` | `.cursor/skills/playwright-*/`, `e2e-testing-patterns/`   | Persistent E2E suite — `@playwright/test` added; `apps/web/e2e/`; `yarn test:e2e` | nice-to-have         | `both` | configured |
| `vitest-midscene-e2e`                                                                             | `.cursor/skills/vitest-midscene-e2e/`                     | AI-driven E2E — needs Midscene wiring (Playwright now available)                  | defer                | `both` | blocked    |
...
| Add E2E test         | `yarn test:e2e` → Playwright (`apps/web/e2e/`); needs running server (`yarn dev`)     |

#### Gaps (follow-up)

- **Playwright added (2026-08-31)** — `@playwright/test` in root devDeps; config at `apps/web/playwright.config.ts`;
  smoke tests at `apps/web/e2e/smoke.spec.ts`; run via `yarn test:e2e`. Not wired into `yarn verify` — needs a running server.
...
| 2026-08-31 | Design & testing skills — phase 1 (docs + rule) | `.cursor/skills/*`, § 2.3               | Configured repo-vendored design/testing cluster: routing rule `.cursor/rules/design-testing-skills.mdc`, registry § 2.3, vendored `walkthrough-artifacts` for Cloud Agents. No Playwright in monorepo — E2E skills marked blocked/defer                                                                                                                                                                                                                                        | Docs-only setup; `yarn verify` not run (host lacks `node_modules`; compose down). Decision tree prioritizes `impeccable` audit + `api-smoke-testing` + `verification-before-completion` for solo builder                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Adopt `@playwright/test` when first E2E gate is planned; re-test `impeccable audit` on a real UI slice                                                                                                                                                                                                                                                                                                  |
| 2026-08-31 | Design & testing skills — phase 2 (Playwright)  | § 2.3, `apps/web/e2e/`, `package.json`  | Added `@playwright/test` to root devDeps; `apps/web/playwright.config.ts` + `apps/web/e2e/smoke.spec.ts` (2-test smoke suite); `yarn test:e2e` at root and in `@aiflow/web`; updated ESLint allowDefaultProject for e2e files; fixed Prettier on 4 files from phase 1; `yarn verify` passes (typecheck + lint + format + 343 unit tests). Playwright skills updated from blocked → configured in § 2.3                                                                         | `yarn verify` exit 0 confirmed. E2E not in verify gate (needs running server). Smoke tests require `BASE_URL=http://localhost:3000 yarn test:e2e` after `docker compose up`. `impeccable audit` untested on real UI slice (deferred)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Add Chromium browser binaries to compose or CI when E2E gate is needed (`npx playwright install chromium`); re-test `impeccable audit` on a real slice                                                                                                                                                                                                                                                  |
