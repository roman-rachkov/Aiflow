...
| A4-03 | ⚠️       | `docs/15` § 2.2 + barrels  | Cross-slice enforced by boundaries; barrel-only `index.ts` imports are convention only — no active `import/no-internal-modules`.                                       | fixed  |
