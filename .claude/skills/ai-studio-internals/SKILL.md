...
**Gitea bootstrap:** compose service `gitea-init` (`docker/gitea/bootstrap.sh`) creates the `GITEA_REPO_OWNER` admin user and writes an API token to volume `gitea_bootstrap` at `/run/gitea/token`. `app` / `worker` read `GITEA_ADMIN_TOKEN_FILE` (fallback: `GITEA_ADMIN_TOKEN`). Required after `docker compose down -v`. Do not commit `GITEA_ADMIN_PASSWORD`; empty means bootstrap generates a local-only password (API uses the token file).
