...
| `docker compose up --scale worker=3` | Scale BullMQ workers (`deploy:run` / `plan:generate` / `code:execute` are real; `spec:generate` is dormant — SPEC generation runs synchronously in the web route, so the queue resolves to a stub-ack) |
...
**Four queues, one worker container each** (`docs/10-infrastructure.md:190`): `spec:generate`, `plan:generate`, `code:execute`, `deploy:run`. Concurrency is 1 per queue. Today three are live (`plan:generate`, `code:execute`, `deploy:run`); `spec:generate` is dormant — SPEC generation runs synchronously in the web route, and the worker falls through to a stub-ack for it.
